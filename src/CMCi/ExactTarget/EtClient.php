...
use SoapFault;
use CMCi\ExactTarget\Exception\EtSoapException;
use CMCi\ExactTarget\Exception\EtClassNotFoundException;
use CMCi\ExactTarget\Exception\EtMethodNotFoundException;
...
class EtClient extends EtBaseClass
{
    ...
    /** @var \CMCi\ExactTarget\EtSoapClient $client */
    protected $client;
    ...
    private $eventProperties = array(
        'sent'             => ['ListID', 'SubscriberID', 'EventDate', 'EventType', 'SubscriberKey', 'SendID'],
        'open'             => ['EventDate', 'EventType', 'SubscriberKey', 'SendID'],
        'click'            => ['EventDate', 'EventType', 'SubscriberKey', 'SendID'],
        'unsub'            => ['EventDate', 'EventType', 'SubscriberKey', 'SendID'],
        'subscriberstatus' => [
            'Client.ID',
            'SubscriberID',
            'SubscriberKey',
     ...
     * @param String $serverInstance Exact Target Server/Instance (e.g. ""; "s4"; "s6"; "s7")
     *
     */
    ...
    public function recall($class, $filter = null, $properties = null, $classNameOverride = null)
    {
        ...
        if (isset($results->Results)) {
            if (is_array($results->Results)) {
                ...
                /**
                 * there are only 1.5k recorders by default;
                 */
                while ($results->OverallStatus=="MoreDataAvailable") {
                    $request->ContinueRequest = $results->RequestID;
                    $requestMsg->RetrieveRequest = $request;
                    $results = null;
                    $results = $this->client->Retrieve($requestMsg);
                    if (isset($results->Results)) {
    ...
    public function update($class, $activeClass, $updateType = "UPDATEADD")
    {
        ...
        $saveOption->SaveAction        = constant("\\CMCi\\ExactTarget\\EtSaveAction::SAVE_".strtoupper($updateType));
        ...
        $objects   = array_map(
            function ($activeClass) use ($nsClass, $class, $className) {
                ...
                return new \SoapVar($activeClass,
                    SOAP_ENC_OBJECT,
                    $className,
                    \CMCi\ExactTarget\EtClient::SOAPWSDL);
            },
            $activeClasses
        );

        $saveOption                    = new \CMCi\ExactTarget\EtSaveOption();
        $saveOption->PropertyName      = $className;
        $saveOption->SaveAction        = constant("EtSaveAction::SAVE_".strtoupper($updateType));
        $requestOptions                = new \CMCi\ExactTarget\EtCreateOptions();
        $requestOptions->SaveOptions[] = new \SoapVar($saveOption, SOAP_ENC_OBJECT, "SaveOption", self::SOAPWSDL);
        $request                       = new \CMCi\ExactTarget\EtCreateRequest();
